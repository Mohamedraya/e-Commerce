...
function cartReducer (state = initialState , action) {
    ...
    switch (action.type) {
        case 'SUCCESS_' + 'fetchCartItems':
            return {...state, cartItems: action.payload.data.items};

        case 'SUCCESS_' + 'makeOrder':
            return {...state, makeOrderSuccess: {} };  
        ...
        case ActionTypes.ADD_PRODUCT_TOCART_ERROR:
            return {...state , addProductToCartError: {errorCode: action.payload.errorCode}};    

        case ActionTypes.UPDATE_CART_ITEM_IMMEDIATLY:
          if (action.payload.action === 'delete') {
             return {...state , 
              cartItems: state.cartItems.filter(item => item._id !== action.payload.cartItemId)};
          }
          else { 
            return {...state , cartItems: state.cartItems.map(item => {
              if (item._id === action.payload.cartItemId){
                return {...item , count :action.payload.action === 'increase'? 
                          item.count + 1
                         :item.count - 1};
              }
              return item;
