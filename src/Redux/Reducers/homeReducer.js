...
function homeReducer (state= initialState , action) {
    switch (action.type) {
        case 'SUCCESS_' + 'fetchHomeCategories': 
        ...
        case 'SUCCESS_' + 'fetchChildrenCategories': 
         return {...state ,  childrenCategories: { ...state.childrenCategories,
                    [action.payload.data.children[0].parentId]:action.payload.data.children
          }                               
        }; 

        case ActionTypes.APPEND_PRODUCTS:
            const catId = action.payload.categoryId;
            return {...state, categoryProducts: {...state.categoryProducts,
                      [catId]: (state.categoryProducts[catId] || [].concat(action.payload.products))
            } };

        case ActionTypes.SET_CATEGORY_PRODUCTS_PAGE:
            return {...state , categoryProductsNextPages:{
        ...
        case ActionTypes.START_FETCHING_CATEGORY_PRODUCTS:
            return {...state , isFetchingCategoryProducts: {
...
export default highOrderReducer(initialState , [
     {
        ...
        baseActionType: 'fetchHomeCategories'
     },
     ...
     {
        requestEndPoint: /^category\/get-children\/[a-zA-Z0-9]+/,
        baseActionType: 'fetchChildrenCategories'
     },

     {
        requestEndPoint: /^product-by-id\/[a-zA-Z0-9]+/,
